    ...
    <div className="flex flex-col gap-3">
      {items.map((item, i) => {
        ...
        return (
          <div
            key={item.title}
            className="overflow-hidden rounded-xl border border-ink-200 bg-white dark:border-white/10 dark:bg-white/[0.03]"
          >
            <button
              type="button"
              onClick={() => setOpenIndex(isOpen ? null : i)}
              aria-expanded={isOpen}
              className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left font-display font-medium text-ink-900 hover:bg-ink-50 dark:text-white dark:hover:bg-white/[0.06]"
            >
